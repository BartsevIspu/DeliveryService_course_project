    ...
    public class VMOrder : VMBase
    {
        ...
        public RelayCommand DeleteOrder
        {
            get
            {
                return deleteOrder ?? (deleteOrder = new RelayCommand(obj =>
                {
                    if (selectedOrder != null)
                    {
                        MessageBoxResult result = System.Windows.MessageBox.Show("Удалить заказ?", "Подтверждение удаления", System.Windows.MessageBoxButton.YesNo);
            ...
            get
            {
                return makeContract ?? (makeContract = new RelayCommand(obj =>
                {
                    if (selectedOrder != null)
                    {
                        ...
                        {
                            MessageBoxResult result = System.Windows.MessageBox.Show("Сначала укажите курьера", "Ошибка", System.Windows.MessageBoxButton.OK);
                        }
                    }
                    else
                    {
        ...
        void ContractCreation()
        {
            try
            {
                ...
                File.Copy("pattern.docx", "Contracts/contract"+selectedOrder.ID+".docx");
            }
            catch(Exception e)
            {
            ...
            DocX doc = DocX.Load("Contracts/contract" + selectedOrder.ID + ".docx");
            ...
            MessageBoxResult res = System.Windows.MessageBox.Show("Договор был оформлен. Название файла: "+ "contract"+selectedOrder.ID+".docx", "Успешно", System.Windows.MessageBoxButton.OK);
